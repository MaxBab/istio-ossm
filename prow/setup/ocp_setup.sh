...
function setup_internal_registry() {
  ...
  if [[ "${TOPOLOGY:-SINGLE_CLUSTER}" != "SINGLE_CLUSTER" ]]; then
    ...
    target_contexts=("$(oc config current-context)")
  fi
...
function deploy_operator(){
  ...
  if [[ "${TOPOLOGY:-SINGLE_CLUSTER}" != "SINGLE_CLUSTER" ]] && [[ ${#cluster_contexts[@]} -gt 0 ]]; then
    ...
    for i in "${!cluster_contexts[@]}"; do
      ...
      oc --context="${ctx}" -n sail-operator wait --for=condition=Available deployment/sail-operator --timeout=240s || { echo "Failed to start sail-operator on ${ctx}"; cleanup_sail_repo; exit 1; }
...
function get_cluster_contexts() {
  ...
  mapfile -t available_contexts < <(oc config get-contexts -o name 2>/dev/null || true)
...
function normalize_kubeconfig_contexts() {
  ...
  for i in "${!kubeconfig_files[@]}"; do
    ...
    local ctx
    ctx=$(oc config get-contexts -o name 2>/dev/null | head -n1)
    file_contexts+=("${ctx}")
  ...
  for i in "${!kubeconfig_files[@]}"; do
    ...
    local current_user
    current_user=$(oc config view -o jsonpath="{.contexts[?(@.name=='${current_context}')].context.user}" 2>/dev/null || echo "")

    # Rename context if needed
    if [ "${current_context}" != "${target_name}" ]; then
      echo "  Renaming context '${current_context}' -> '${target_name}' in ${kconfig}"
      oc config rename-context "${current_context}" "${target_name}" 2>/dev/null || {
...
function setup_ocp_multicluster_topology() {
  ...
  for i in "${!CLUSTER_NAMES[@]}"; do
    ...
    for kconfig in "${kubeconfig_paths[@]}"; do
      if oc --kubeconfig="${kconfig}" config get-contexts "${cluster_context}" &>/dev/null; then
