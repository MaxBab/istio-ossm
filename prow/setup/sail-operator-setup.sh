...
function switch_cluster() {
  ...
  oc config use-context "$cluster_name" >/dev/null || {
    echo "Error: failed to switch to context '$cluster_name'"
    exit 1
  }
  echo "Switched to cluster: $cluster_name (KUBECONFIG=${KUBECONFIG:-unset})"
}
